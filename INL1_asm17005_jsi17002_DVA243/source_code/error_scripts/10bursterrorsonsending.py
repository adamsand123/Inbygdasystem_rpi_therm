                ...
                if temp >= 26 and not isOn:
                        try:
                                while not answer == 1: #keep sending until we get a 1 back from the other rpi device, to assure that he recieved the msg
                                        if burstTime:
                                                print "sending burst " + str(datetime.datetime.now())
                                                for i in range(10):
                                                        print(str(i))
                                                        s.sendto("5", adress2)
                                                burstTime = False
...
def tempCheck():
        ...
        while True:
                ...
                if temp >= 26 and not checkTimeOnce:
                        print "The temprature was messearued to be 26 C or higher at: " + str(datetime.datetime.now())
                        checkTimeOnce = True
